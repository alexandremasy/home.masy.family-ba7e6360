  ...
  const label = (offset: number) =>
    offset === 0
      ...
      : offset === 1
        ...
        : offset === 2
          ? "Après-demain"
          : frLongDay(addDays(TODAY, offset));
...
function NetBlock({
  ...
  return (
    <div className="rounded-xl bg-secondary/60 p-2.5 sm:p-3 transition-colors min-w-0">
      ...
      <Eyebrow tone="current" size="xs" className="mt-1 truncate text-muted-foreground/80">
        {foot}
      </Eyebrow>
    </div>
  );
...
function BlockShell({
  ...
  return (
    <div
      className={
        "rounded-xl p-2.5 sm:p-3 transition-colors min-w-0 " +
  ...
  return (
    <BlockShell
      ...
      icon={
        <Flame
          className={
            "h-3.5 w-3.5 sm:h-4 sm:w-4 " + (low ? "anim-wiggle text-warm" : "anim-breathe")
          }
        />
      }
      label="Mazout"
    >
      ...
      <Tip label={`Niveau visualisé : ${o.tankPct}% de la capacité totale.`}>
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted">
          <div
            className={
              "h-full rounded-full transition-all duration-700 " + (low ? "bg-warm" : "bg-primary")
            }
            style={{ width: `${o.tankPct}%` }}
          />
        </div>
      </Tip>
      <div className="mt-2 flex flex-col gap-1 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        ...
        <Tip label="Autonomie estimée à ce rythme de consommation.">
          <span className={"tabular-nums " + (low ? "font-semibold text-warm" : "")}>
            ~{o.autonomyDays} j
          </span>
        </Tip>
      </div>
...
function RepasLine() {
  ...
  const dateLabel = TODAY.toLocaleDateString("fr-BE", {
    weekday: "long",
    day: "numeric",
    month: "long",
  });
    ...
    >
      ...
      {today.midi || today.soir ? (
        <div className="mt-2 space-y-1.5">
          {today.midi && (
            <MealRow icon={<Sun className="h-3.5 w-3.5 text-mustard" />} name={today.midi} />
          )}
          {today.soir && (
            <MealRow icon={<Moon className="h-3.5 w-3.5 text-primary" />} name={today.soir} />
          )}
        </div>
      ) : (
