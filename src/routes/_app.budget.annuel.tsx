  ...
  const zoomData = useMemo(
    () =>
      MONTHS_FR.map((m, i) => ({
        m,
        v: Math.round(
          zoom.actual * (0.85 + Math.sin(i * 0.9 + zoomCat.length) * 0.18 + (i === 9 ? 0.4 : 0)),
        ),
      })),
    [zoom, zoomCat],
  );
                ...
                <div className="flex items-center justify-between">
                  ...
                  {cost > 0 && (
                    <span
                      ...
                      style={{
                        background: `color-mix(in oklab, var(--mustard) ${30 + intensity * 70}%, transparent)`,
                      }}
                    />
                  )}
                </div>
                <div className="mt-1.5 flex flex-1 flex-col gap-1">
                  ...
                  {bills.map((b) => (
                    <span
                      ...
                      className={
                        "truncate rounded-md px-1.5 py-0.5 text-2xs font-semibold leading-tight " +
                        (b.kind === "income"
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {envelopes.map((env) => {
            const data = Array.from({ length: 12 }, (_, i) => ({
              i,
              v:
                Math.max(0, env.balance - env.contrib * (11 - i)) +
                Math.sin(i * 1.3) * env.contrib * 0.15,
            }));
            const toneRing =
