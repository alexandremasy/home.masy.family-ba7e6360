...
function ReserveOverlay() {
  ...
  const savAxis = useYAxis(
    [...savings.series.flatMap((s) => [s.reel ?? 0, s.proj ?? 0]), savings.floor],
    5000,
  );
  ...
  const startEdit = (key: string, bal: number, contrib: number) => {
    setEditKey(key);
    setDraftBal(String(bal));
    setDraftContrib(String(contrib));
  };
  const commitEdit = (key: string) => {
    const bal = Number(draftBal.replace(",", "."));
    const contrib = Number(draftContrib.replace(",", "."));
    if (![bal, contrib].every((n) => Number.isFinite(n) && n >= 0)) {
              ...
              <Section
                ...
                action={
                  ...
                  >
                    <span
                      className={
                        "h-1.5 w-1.5 rounded-full " + (belowFloor ? "bg-destructive" : "bg-success")
                      }
                    />
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        tickLine={false}
                        axisLine={false}
                        domain={[0, savAxis.yTop]}
                        ticks={savAxis.yTicks}
                      ...
                      <ReferenceLine
                        ...
                        strokeDasharray="4 4"
                        label={{
                          value: `Seuil sain · ${eur(savings.floor)}`,
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          {!editing && (
                            <button
                              onClick={() => startEdit(env.key, env.balance, env.contrib)}
