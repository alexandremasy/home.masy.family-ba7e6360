...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-2 border-b border-border/40 py-4 last:border-0 sm:grid-cols-[10rem_minmax(0,1fr)] sm:gap-4">
      <Eyebrow size="xs" className="pt-1.5">
        {label}
      </Eyebrow>
    ...
    <div className="space-y-8 pb-16">
      ...
      <Section title="Colour">
        <p className="mb-5 text-sm text-muted-foreground">
          ...
          page — so each swatch below <b>is</b> a rung of one of the nine ramps. Values are read
          live off <code className="font-mono text-xs">document.documentElement</code>; flip the
          theme and they follow. <b>The dark theme is not from Figma</b> — the library has no dark
          ramp, so it stays as authored. That's the one place a colour here is still invented.
        </p>

        <div className="space-y-6">
          <div>
            <Eyebrow size="xs" className="mb-3">
              Semantics — meaning before hue
            </Eyebrow>
        ...
        <Panel padding="sm" className="mt-6 border-warm/40 bg-warm/5">
          <Eyebrow size="xs" tone="current" className="text-warm">
            The rule
          </Eyebrow>
          <p className="mt-1.5 text-sm">
            A <code className="font-mono text-xs">-foreground</code> token only works on its own
            solid fill. Since the port, every accent foreground is <b>Light/90</b> (dark) in both
            themes, so they no longer invert — they were chosen for contrast: warm 6.4:1, mustard
            9.0:1, teal 5.1:1. On a <i>tint</i> like{" "}
            <code className="font-mono text-xs">bg-warm/15</code> the fill is nearly the page, so
            use <code className="font-mono text-xs">text-warm</code> there, not the foreground.
          </p>
      ...
      <Section title="Type">
        <p className="mb-5 text-sm text-muted-foreground">
          The scale is <b>ported from Figma</b> — <i>alexandremasy — tokens</i>, node 503-473 —
          which is the source of truth. Barlow, eleven steps, nothing else. Every size below is
          measured off its own rendered node, like the swatches above, so this page can't drift from{" "}
        ...
        <div className="space-y-6">
          <div>
            <Eyebrow size="xs" className="mb-1">
              The scale — 10 · 12 · 14 · 16 · 20 · 24 · 28 · 32 · 40 · 48 · 56
            </Eyebrow>
            <p className="mb-3 text-xs text-muted-foreground">
              <code className="font-mono">styles.css</code> wipes Tailwind's own scale (
              <code className="font-mono">--text-*: initial</code>) before declaring these, so they
              are the only sizes that exist. <code className="font-mono">text-7xl</code> and{" "}
              <code className="font-mono">text-[13px]</code> aren't discouraged — they're
              unwritable.
            </p>
            <div>
              ...
              <TypeRow
                cls="text-sm"
                role="14 · body, the default"
                sample="Reads at arm's length."
              />
              <TypeRow
                cls="text-xs"
          ...
          <div>
            <Eyebrow size="xs" className="mb-1">
              Two things here are not from Figma
            </Eyebrow>
            <div className="grid gap-3 sm:grid-cols-2">
              <Panel padding="sm">
                ...
                <p className="mt-1.5 text-xs text-muted-foreground">
                  ...
                  leading and a paragraph you can't read. These keep Tailwind's rhythm — generous at
                  body sizes, ~1 at display sizes. Deliberate divergence.
                </p>
              </Panel>
              <Panel padding="sm">
        ...
        <Panel padding="sm" className="mt-6 border-warm/40 bg-warm/5">
          ...
          <p className="mt-1.5 text-sm">
            ...
            <code className="font-mono text-xs">text-xl</code> is <b>24px</b> here, not Tailwind's
            20. Six rungs already coincided (12·14·16·20·24·48), which is why the app was mostly on
            this rhythm before the port. Read the value, not the name — or read this page.
          </p>
        </Panel>
      </Section>
      ...
      <Section title="Buttons">
        ...
        <Row label="Sizes">
          ...
          <Button size="iconRound" variant="outline">
        ...
        <Row label="States">
          ...
          <Button variant="outline" size="iconRound" disabled>
            <ChevronRight className="h-4 w-4" />
          </Button>
          <CommandButton
        ...
        <p className="pt-3 text-xs text-muted-foreground">
          <b>inverted</b> is this app's primary action —{" "}
          <code className="font-mono">bg-foreground</code>, not the teal of{" "}
          <code className="font-mono">--primary</code>. <b>iconRound</b> exists because{" "}
          <code className="font-mono">size="icon"</code> ships as{" "}
      ...
      <Section title="Tags & pills">
        ...
        <Row label="StatusPill">
          <StatusPill tone="primary" icon={<Repeat className="h-3 w-3" />}>
            acted on · 1 slot
          </StatusPill>
        ...
        <p className="pt-3 text-xs text-muted-foreground">
          ...
          <code className="font-mono">muted</code> for an annotation. Never{" "}
          <code className="font-mono">warm</code>.
        </p>
      </Section>
      ...
      <Section title="Form">
        ...
        <Row label="Select">
          <Select defaultValue="chili">
            <SelectTrigger aria-label="Select demo" className="w-56">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <Row label="Tabs">
          <Tabs defaultValue="a">
            <TabsList className="h-10 bg-secondary/70 p-1">
              <TabsTrigger value="a" className="px-4">
                Overview
              </TabsTrigger>
              <TabsTrigger value="b" className="px-4">
                Readings
              </TabsTrigger>
              <TabsTrigger value="c" className="px-4">
                Import
              </TabsTrigger>
            </TabsList>
          </Tabs>
      ...
      <Section title="Containers">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Panel padding="sm">
            <Eyebrow size="xs">Panel</Eyebrow>
            <p className="mt-1 text-sm">padding sm</p>
          </Panel>
          <Panel>
            <Eyebrow size="xs">Panel</Eyebrow>
            <p className="mt-1 text-sm">padding md (default)</p>
          </Panel>
          <Panel padding="lg">
            <Eyebrow size="xs">Panel</Eyebrow>
            <p className="mt-1 text-sm">padding lg</p>
          </Panel>
        </div>
        <Eyebrow size="xs" className="mb-3 mt-6">
          Tile — the bento tones
        </Eyebrow>
        <div className="grid-bento">
          {(["default", "primary", "mustard", "warm", "dark"] as const).map((t) => (
            <Tile key={t} span={1} tone={t}>
              <Eyebrow size="xs" tone="current" className="opacity-70">
                tone
              </Eyebrow>
        ...
        <Row label="DishCard">
          ...
          <div className="w-40 rounded-lg bg-secondary/50 p-2">
            <DishCard
              dish={dish}
              variant="compact"
              status={<StatusPill icon={<Repeat className="h-3 w-3" />} title="batch" />}
            />
          </div>
        </Row>
        <Row label="BudgetBar">
          <div className="w-full max-w-md space-y-3">
            <div>
              <p className="text-xs text-muted-foreground">60% spent</p>
              <BudgetBar value={60} />
            </div>
            <div>
              <p className="text-xs text-muted-foreground">actual 40% · projected 80%</p>
              <BudgetBar value={40} projected={80} />
            </div>
            <div>
              <p className="text-xs text-muted-foreground">
                overrun — the only legitimate warm on this page
              </p>
              <BudgetBar value={100} overflow={18} />
            </div>
          </div>
        </Row>
      ...
      <Section title="Overlays">
        <Row label="Dialog">
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle className="font-serif text-lg">A Dialog</DialogTitle>
              </DialogHeader>
          ...
          <Drawer>
            ...
            <DrawerContent className="bg-background px-4 pb-6">
              <DrawerHeader className="px-0 text-left">
                <DrawerTitle className="font-serif text-lg">A Drawer</DrawerTitle>
              </DrawerHeader>
              <p className="text-sm text-muted-foreground">
                From the bottom, drag to dismiss. This is the meal picker on mobile.
              </p>
            </DrawerContent>
          </Drawer>
        </Row>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  A confirmation says what gets lost, not "are you sure".
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <Row label="DropdownMenu">
          ...
          <span className="text-xs text-muted-foreground">
            keyboard highlight is neutral now, not mustard
          </span>
        </Row>
        <Row label="Tooltip">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="iconRound">
            ...
            <TableBody>
              {dishes.slice(0, 3).map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell className="px-3 py-2.5 capitalize text-muted-foreground">
                    {d.base}
                  </TableCell>
      ...
      <Panel padding="sm" className="border-dashed">
        <p className="text-xs text-muted-foreground">
          Missing a control?{" "}
          <b>
            Look in <code className="font-mono">src/components/ui/</code> first
          </b>{" "}
          — 19 primitives, all of them live. It exists but fights the palette?{" "}
          <b>
            Edit <code className="font-mono">ui/*</code>
          </b>
          , we own that code. It really is this app's own vocabulary? Build it <b>once</b> in{" "}
          <code className="font-mono">components/</code> and share it. Never a fourth path. See{" "}
