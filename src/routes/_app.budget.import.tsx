      ...
      {stage !== "idle" && (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-card px-4 py-3 shadow-soft">
            ...
            <button onClick={reset} className="rounded-full px-3 py-1.5 text-xs text-muted-foreground hover:bg-secondary hover:text-foreground">Changer de fichier</button>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            <StatBadge icon={Lock} tone="accent" n={t.protegees} label="Protégées" />
          </div>
          ...
          {/* Nouvelles */}
          <DiffSection title="Nouvelles — à ajouter" subtitle="Lignes encore jamais vues. Elles seront ajoutées à la validation.">
            <ul className="divide-y divide-border/40">
          ...
          {stage === "committed" && (
            <div className="rounded-2xl border border-success/30 bg-success/5 p-6 text-center anim-pop-in">
              ...
              <p className="mt-3 font-serif text-2xl tracking-tight">Import appliqué</p>
              <p className="mt-1 text-sm text-muted-foreground">
                {t.nouvelles} transactions ajoutées, {Object.values(choices).filter((c) => c === "maj").length} mises à jour.
              </p>
              <button onClick={reset} className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-border/60 bg-background px-4 py-2 text-sm hover:bg-secondary">
          ...
          {importHistory.map((h) => (
            <li key={h.filename} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm">
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground tabular-nums">
                <span className="rounded-full bg-success/10 px-2 py-0.5 text-success">+{h.added} ajoutées</span>
                <span className="rounded-full bg-warm/10 px-2 py-0.5 text-warm">{h.updated} màj</span>
              </div>
            </li>
          ))}
        </ul>
...
function DropZone({ dragOver, setDragOver, onDrop }: {
  dragOver: boolean; setDragOver: (v: boolean) => void; onDrop: (f?: File) => void;
}) {
  return (
    <label
      ...
      className={"flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed bg-card p-12 text-center transition-all duration-300 " +
        (dragOver ? "border-primary bg-primary/5 scale-[1.01]" : "border-border/60 hover:border-foreground/30 hover:bg-secondary/30")}
    >
      <span className={"grid h-14 w-14 place-items-center rounded-full transition-colors " + (dragOver ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary")}>
        <UploadCloud className="h-6 w-6" />
      </span>
      <div>
        <p className="font-serif text-xl tracking-tight">Déposez un export ici</p>
        <p className="mt-1 text-sm text-muted-foreground">.xlsx ou .csv depuis iSaveMoney — ou cliquez pour parcourir</p>
      </div>
...
function StatBadge({ icon: Icon, tone, n, label }: {
  icon: typeof Plus; tone: "success" | "muted" | "warm" | "accent"; n: number; label: string;
}) {
  const cls =
    tone === "success" ? "bg-success/10 text-success border-success/20"
    : tone === "warm" ? "bg-warm/10 text-warm border-warm/20"
    : tone === "accent" ? "bg-accent/15 text-accent-foreground border-accent/30"
