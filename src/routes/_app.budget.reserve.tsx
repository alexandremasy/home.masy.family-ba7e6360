...
export const Route = createFileRoute("/_app/budget/reserve")({
  component: ReservePage,
  // Reached from a verdict box on /budget/vue; carries the same view so the numbers match.
  validateSearch: (s: Record<string, unknown>): { view: BudgetView } => ({
    view: typeof s.view === "number" ? s.view : s.view === "rolling" ? "rolling" : "rolling",
  }),
  head: () => ({ meta: [{ title: "Réserve — Budget" }] }),
});
...
function ReservePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Same page chrome as a room page: sticky header + back link */}
