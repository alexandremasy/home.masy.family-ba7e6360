...
import {
  ShieldCheck, DoorOpen, Lock, Bell, Package, User, Car, Cat,
  Activity as ActivityIcon, Wifi, WifiOff, Siren, BatteryLow, Radio,
} from "lucide-react";
...
function HealthStat({ icon, label, value, sub, tone = "default" }: {
  ...
  return (
    <div className={`rounded-xl border p-4 ${tone === "warm" ? "border-warm/40 bg-warm/10" : "border-border/60 bg-card"}`}>
      <div className={`flex items-center gap-2 text-xs uppercase tracking-eyebrow ${tone === "warm" ? "text-warm" : "text-muted-foreground"}`}>
        {icon}{label}
      </div>
      <p className="mt-2 font-serif text-xl">{value}{sub && <span className="ml-1 text-sm text-muted-foreground">{sub}</span>}</p>
    </div>
  );
}
