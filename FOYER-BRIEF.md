...
- **Trois leviers de répétition, distincts [ne pas confondre — détaillés dans « Cohérence multi-jour »].** (a) **Cadence** = attribut du plat, espacement de variété entre fenêtres ; (b) **rendement / batch** = un gros plat cuisiné 1× couvre N slots ; (c) **écoulement de composant** = réutiliser un modifier acheté en gros. Pas de règle « N semaines » globale — c'est un équilibrage par rôle sur la fenêtre.
...
**Le nœud — dénoué : matière libre, pas de taxonomie [résolution].** Alex refuse (à raison) de figer une liste de méta depuis un cas : « c'est différent pour chaque personne, je ne tire pas une règle sur une itération ». Ce constat **est** la réponse — le système n'impose **aucun schéma de méta rigide**. Deux niveaux :
- **Méta structurées minimales** (mécanique, pas personnalité) : date de naissance (= le trigger) · langue · cap d'âge (dérivé de la date, non saisi) · position par défaut des curseurs · **historique des messages déjà générés** (anti-répétition d'une année sur l'autre).
- **Matière libre par personne** : une zone de notes en vrac — souvenirs, private jokes, ce qu'on aime chez elle, sa vie du moment — **différente pour chacun, sans champ imposé**. C'est LA source de personnalisation. Un LLM digère mieux du texte libre qu'un formulaire à cases, et ça reste léger (« pas de couches »).

Le générateur reçoit : **matière libre + curseurs + socle (jamais bateau) + historique** → message, régénérable par commentaire. C'est le système qui s'adapte à chaque personne — Alex ne généralise pas.
